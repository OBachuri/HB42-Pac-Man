...
class LevelWeb:
    def __init__(self, data: dict[str, Any] = {}) -> None:
        ...
        self.speed_factor_player: float = data.setdefault("speed_factor_player",
                                                          0.01)
        self.speed_factor_ghost: float = data.setdefault("speed_factor_ghost",
                                                         0.02)


class ConfigWeb:
    def __init__(self, cfg_data: dict[str, Any] = {}) -> None:
        self.highscores_filename: str = cfg_data.setdefault(
            "highscores_filename", "highscores.json")
        self.lives: int = cfg_data.setdefault("lives", 3)
        self.points_per_pacgum: int = cfg_data.setdefault(
            "points_per_pacgum", 10)
        self.points_per_super_pacgum: int = cfg_data.setdefault(
            "points_per_super_pacgum", 50)
        self.points_per_ghost: int = cfg_data.setdefault("points_per_ghost",
                                                         200)
        self.seed: int = cfg_data.setdefault("seed", 42)
        self.cheat: bool = cfg_data.setdefault("cheat", False)

        levels: list[LevelWeb] = [LevelWeb(level_data) for level_data
                                  in cfg_data.setdefault("levels", [])]
